...
import { saveCv, saveInterests, saveAvatar } from "@/app/actions/profile"
...
export function ProfileExtras({
  ...
  async function onPickCv(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const path = user.id + "/cv-" + Date.now() + "-" + file.name
      ...
      if (!json.ok) throw new Error(json.error || "Gagal membaca CV")
      await saveCv({ cv_url: pub.publicUrl, cv_text: json.text })
  ...
  return (
    <div className="iq-card iq-card__pad">
      <h3 className="mb-4">CV &amp; Preferensi</h3>
      ...
      <div className="iq-form-row">
        ...
        <div
          className={"iq-cvbox" + (cvLabel ? " is-set" : "")}
          onClick={() => !cvBusy && cvRef.current?.click()}
        >
          {cvBusy
            ? "Membaca CV dengan AI…"
            : cvLabel
            ? "✓ " + cvLabel + " · klik untuk ganti"
            : "Klik untuk upload CV (PDF)"}
        </div>
